...
PR本文は、実改行を含む UTF-8（BOM なし）の Markdown ファイルを `gh pr create --body-file <本文ファイル>` で渡す。JSON/JavaScript のエスケープ済み文字列を `--body` に渡したり、本文中に `\n` を埋め込んだりしない。Windows / PowerShell でも、見出し・箇条書き・コードスパンを通常の Markdown として記述し、`\n`、`\r`、`\t` などのバックスラッシュ表記を改行や制御文字の代わりに使わない。

このリポジトリでは、PRの作成・更新に `scripts/pr.ps1` を使う。作成は `scripts/pr.ps1 -Action create -Title <タイトル> -BodyFile <本文ファイル>`、更新は `scripts/pr.ps1 -Action edit -Number <番号> -BodyFile <本文ファイル>` とし、同スクリプトのUTF-8/制御文字検査とGitHubからの読み戻し一致確認を通す。

PR作成・更新後は `gh pr view <番号> --json body --jq .body` で本文を読み戻して確認する。見出しが独立行にあり、本文に文字列 `\n` や置換文字 `�` が混入していないことを確認してから作業を完了する。

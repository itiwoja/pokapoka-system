...
`questions[]` のアレルギー設問回答と `special_request` は、中継サーバーで
`allergies`（安全情報）と `request`（その他の要望）へ正規化され、予約ストックと
予約を進行中へ移した後の厨房カードへ表示される。設問の実際の質問文・回答形式は
店舗設定によって異なるため、本番キー取得後に実レスポンスでも確認する。
